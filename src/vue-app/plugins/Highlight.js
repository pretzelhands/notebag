...
function getDecorations({ doc, name }) {
	...
	blocks.forEach(block => {
		...
		const language = block.node.attrs.language || "auto";
		const nodes = language === "auto"
	...
	return new Plugin({
		...
		state: {
			...
			apply: (transaction, decorationSet, oldState, state) => {
				...
				if (transaction.docChanged && [nodeName, previousNodeName].includes(name) || transaction.updated === 1) {
